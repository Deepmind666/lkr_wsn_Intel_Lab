"""
内存优化的真实数据WSN路由系统 (修复版)
解决大数据集内存不足问题和OpenMP冲突
"""
...
class MemoryEfficientConfig:
    ...
    batch_size: int = 256  # 减小批处理大小
    sample_ratio: float = 0.05  # 进一步减少数据采样比例
    max_nodes: int = 15  # 减少最大使用节点数
...
class RealDataLoader:
    ...
    def load_intel_data(self) -> bool:
        ...
        try:
            ...
            # 读取数据，使用更小的chunksize
            chunks = []
            chunk_size = 50000  # 减小chunk大小
            max_chunks = 5  # 最多读取5个chunk
            
            for i, chunk in enumerate(pd.read_csv(data_file, sep=' ', header=None, chunksize=chunk_size)):
                if i >= max_chunks:
                    break
...
class MemoryEfficientWSNSystem:
    ...
    def __init__(self, config: MemoryEfficientConfig):
        ...
        self.training_history = {
            'lstm_losses': [],
            'data_source': 'Intel Berkeley Lab (Memory Optimized Fixed)',
            'training_samples': 0,
            'nodes_used': 0
        }
    ...
    def prepare_training_data(self) -> Tuple[Optional[DataLoader], Optional[DataLoader]]:
        ...
        for node_id in valid_nodes:
            ...
            if max_sequences < 10:
                max_sequences = min(10, len(feature_data) - self.config.sequence_length)
    ...
    def train_model(self, epochs=20) -> bool:
        """训练模型"""
    ...
    def save_results(self):
        """保存结果"""
        results_dir = Path(__file__).parent.parent.parent / "results" / "memory_efficient_wsn_fixed"
        results_dir.mkdir(parents=True, exist_ok=True)
        
        # 保存指标
        with open(results_dir / "fixed_metrics.json", 'w', encoding='utf-8') as f:
            json.dump(self.metrics, f, indent=2, ensure_ascii=False)
        
        # 保存训练历史
        with open(results_dir / "fixed_training_history.json", 'w', encoding='utf-8') as f:
            json.dump(self.training_history, f, indent=2)
        
        # 保存模型
        if self.model:
            torch.save(self.model.state_dict(), results_dir / "fixed_lstm_model.pth")
    ...
    def visualize_results(self):
        ...
        try:
            ...
            if self.training_history['lstm_losses']:
                ...
                axes[0, 0].set_title('LSTM训练损失 (修复版)')
            ...
            plt.suptitle('内存优化的真实数据WSN分析 (修复版)', fontsize=14)
            plt.tight_layout()
            plt.savefig(results_dir / 'fixed_results.png', dpi=300, bbox_inches='tight')
            plt.close()
...
def main():
    """主函数"""
    logger.info("🚀 启动内存优化的真实数据WSN系统 (修复版)")
    ...
    # 训练模型
    if wsn_system.train_model(epochs=20):
        logger.info("✅ 模型训练成功")
    else:
        logger.error("❌ 模型训练失败")
        return
    ...
    logger.info("✅ 内存优化WSN系统运行完成 (修复版)")
    logger.info("📊 使用真实Intel Berkeley数据集")
    logger.info("🔧 优化措施：数据采样、批处理、节点限制、OpenMP修复")
